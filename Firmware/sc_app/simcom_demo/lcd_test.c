/**
  ...
  * @brief   Source file of lcd test for SPI LCD driver,ST7735S 128x160 for example.
  ******************************************************************************
  * @attention
  *
...
0x12,0x24,0x12,0xB4,0x13,0x28,0x12,0x20,0x10,0x20,0x28,0x20,0x47,0xFE,0x00,0x00,/*"ӭ",1*/
"ʹ",0x10,0x40,0x10,0x40,0x1F,0xFE,0x20,0x40,0x20,0x40,0x67,0xFC,0x64,0x44,0xA4,0x44,
0x27,0xFC,0x20,0x40,0x22,0x40,0x21,0x40,0x20,0x80,0x21,0x40,0x22,0x30,0x2C,0x0E,/*"ʹ",2*/
"��",0x00,0x00,0x3F,0xF8,0x21,0x08,0x21,0x08,0x21,0x08,0x3F,0xF8,0x21,0x08,0x21,0x08,
...
/**
  * @brief  Display gbk 16x16 string
...
void show_gbk_16x16_test(UINT16 x, UINT16 y, UINT16 fc, UINT16 bc, UINT8 *s)
{
    ...
    while(len)
    {
        while(*s)
        {
            if((*s) < 128)//�����ASCII�룬��ô���С��128
            {
                ...
                else
                {
                    if(k>32)
                        ...
                        k=0;//ASCIIǰ32���������ַ������˻س�������ʾû��Ӱ�죬�����ֿⲻ����
                    for(i=0;i<16;i++)
                    {
                        for(j=0;j<8;j++)
                        {
                            if(ASC16[k*16+i]&(0x80>>j))//�������ݣ�������  �Ӹ�λ����λȡ���ַ�
                            {
            ...
            {
                for(k=0;k<HZ16_num;k++) //���������ֿ�
                {
                    if((HZ16[k].Index[0]==*(s))&&(HZ16[k].Index[1]==*(s+1)))  //�����ҵ�������˵���ֿ����д˺��֡�һ������ռ�����ֽ�
                    {
                        for(i=0;i<16;i++)
                        {
                            for(j=0;j<8;j++)
...
/**
  * @brief  Clear LCD screen with fixed color data
  * @param  FillData,data to be filled.
  * @note   
  * @retval void
  */
