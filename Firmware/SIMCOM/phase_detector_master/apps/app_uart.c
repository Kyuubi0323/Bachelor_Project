...
static UINT8 Cal_CheckSumU8(UINT8 *buf, UINT16 len)
{
	UINT8 csum = 0;
	UINT16 i = 0;
	for (i = 0; i < len; i++) {
...
void parseData(UINT8 *data, UINT16 size) {
    ...
    phaseData.seq = data[15];
    logi("ts:%lu, %d, %d, %d, %d", phaseData.ts, phaseData.l1, phaseData.l2, phaseData.l3, phaseData.seq);
