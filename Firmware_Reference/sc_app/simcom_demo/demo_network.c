            ...
            case 4:
            {
                ...
                if(ret == SC_NET_SUCCESS)
                {
                    sAPI_Debug("Get cpsi success. NEmode=%s,MM=%s,LAC=%d,CELL=%d,Gband=%s,Lband=%s,TAC=%d,RSRP=%d,RXLEV=%d,TA=%d!",
                               Scpsi.networkmode,Scpsi.Mnc_Mcc,Scpsi.LAC,Scpsi.CellID,Scpsi.GSMBandStr,Scpsi.LTEBandStr,Scpsi.TAC,Scpsi.Rsrp,Scpsi.RXLEV,
                               Scpsi.TA);
                    sprintf(NetResp,"\r\nGet cpsi success. NEmode=%s,MM=%s,LAC=%d,CELL=%d,Gband=%s,Lband=%s,TAC=%d,RSRP=%d,RXLEV=%d,TA=%d,SINR=%d,rsrq=%d,rssi=%d!\r\n",
                               Scpsi.networkmode,Scpsi.Mnc_Mcc,Scpsi.LAC,Scpsi.CellID,Scpsi.GSMBandStr,Scpsi.LTEBandStr,Scpsi.TAC,Scpsi.Rsrp,Scpsi.RXLEV,
                               Scpsi.TA,Scpsi.SINR,Scpsi.Rsrq,Scpsi.Rssi);
                    PrintfResp(NetResp);
                    break;
                }
                else
...
/**
  * @brief  Netlight pin control Demo
  * @param  void
  * @note   
  * @retval void
  */
